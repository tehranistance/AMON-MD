...
export default {
    ...
    category: 'general',
    description: 'Get session id for AMON-MD',
    usage: '.pair 25475900XXXX',
    async handler(sock, message, args, context) {
        ...
        try {
            const response = await axios.get(`https://gaga-md-session-pair.pxxl.click/code?number=${number}`, {
                timeout: 60000
            });
            if (response.data && response.data.code) {
                ...
                const successText = `✅ *AMON-MD PAIRING CODE*\n\n` +
                    `Code: *${pairingCode}*\n\n` +
                    `*How to use:*\n` +
